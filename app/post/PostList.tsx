    ...
    <section className="flex flex-col items-center justify-center">
      ...
      <div className="grid w-5/6 max-w-5xl grid-cols-2 gap-4 md:grid-cols-3">
        {posts.items.map((post: IPostItem) => {
          if (!targetTag) return <PostCard post={post} key={post._id} href={`/post/${post._id}`} />
          return post.tags.includes(targetTag) ? (
            <PostCard post={post} key={post._id} href={`/post/${post._id}`} />
          ) : null
        })}
      </div>
